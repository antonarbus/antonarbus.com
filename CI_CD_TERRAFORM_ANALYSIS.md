### 2. No Terraform Plan Visibility Before Apply

**Location**: `/Users/sherb/Git/antonarbus.com/.github/workflows/deploy.yml:122-143`

**Problem**:
Workflow runs `terraform apply -auto-approve` directly without showing plan output for review.

**Current Flow**:

```bash
./terraform.sh  # Runs terraform apply -auto-approve immediately
```

**Risk**:

- Infrastructure changes applied blindly
- No visibility into what resources will be created/modified/destroyed
- No human review before destructive operations
- Hard to debug when something goes wrong
- Can't catch mistakes before they're deployed

**Best Practice Flow**:

```bash
terraform plan -out=tfplan     # Generate and review plan
...
- name: 'Terraform Plan'
  ...
  run: |
    cd infrastructure/
    terraform plan -no-color | tee /tmp/tfplan.txt

    echo "## 📋 Terraform Plan" >> $GITHUB_STEP_SUMMARY
    echo "" >> $GITHUB_STEP_SUMMARY
    echo '```terraform' >> $GITHUB_STEP_SUMMARY
    cat /tmp/tfplan.txt >> $GITHUB_STEP_SUMMARY
    echo '```' >> $GITHUB_STEP_SUMMARY
  working-directory: ${{ env.TERRAFORM_DIR }}
````

Add this BEFORE the "Terraform Apply with Smart Bootstrap" step.

**Estimated Time**: 1 hour

---

### 3. Misleading Git Commit Message in History

**Location**: Git commit `78a7c46`

**Problem**:

- Commit message says: "update GCS bucket name to 'antonarbus-terraform-state-test'"
- This was later reverted to production name `antonarbus-terraform-state` in commit `51a3bc1`
- Git history suggests test bucket is in use when production bucket is actually configured

**Impact**:

- Confusing when reviewing git history
- May cause debugging issues if someone looks at commit messages
- Suggests the infrastructure uses a test bucket when it doesn't

**Current State**:

- ✅ Code is correct (uses production bucket)
- ❌ Git history is misleading

**Fix Options**:

1. **Leave as-is**: Code is correct, just be aware git history is misleading
2. **Document in commit**: Add a follow-up commit explaining the production bucket is the actual configuration
3. **Update .mailmap or git notes**: Add context to commits (advanced)

**Recommendation**: Document this in infrastructure README.md or leave as-is since current state is correct.

**Estimated Time**: 10 minutes (if documenting)
