    ...
    <>
      <ul>
        <li>
          ...
          <Lnk path="https://www.youtube.com/watch?v=c3JGBdxfYcU">this</Lnk> &{' '}
          <Lnk path="https://www.youtube.com/watch?v=UUga4-z7b6s">this</Lnk>
        </li>
        <li>
          this is not only about folder structures, but also how modules interact with each other in
          that folders
        </li>
      ...
      <LazyImg
        path="/imgs/frontend-architecture/01 01 classical no architecture.png"
        width="200px"
      />
      ...
      <ul>
        <li>
          <code>modules</code> inside have business oriented segments, which do not interact with
          each other
        </li>
        <li>
          <code>module</code> is responsible for one task & ideally has all needed to do the task
        </li>
      ...
      <LazyImg
        path="/imgs/frontend-architecture/02 02 data flows from to to bottom.png"
        width="400px"
      />
      ...
      <p>
        <code>UI</code> layer has most generic UI blocks without any logic
      </p>
      ...
      <ul>
        <li>
          <code>components</code> layer contains a bit more complex components that may use blocks
          from <code>ui</code>
        </li>
      ...
      <ul>
        <li>
          <code>modules</code> layer has isolated independent parts with its own responsibilities
        </li>
      ...
      <ul>
        <li>
          <code>module</code> contains all needed data and functionality inside to be independent
          from other modules
        </li>
        <li>
          <code>modules</code> do not interact with each other
        </li>
        <li>
          all logic is encapsulated inside a module and data to the outside should go via public
          apis <code>index</code> file
        </li>
      </ul>
      ...
      <p>
        <code>pages</code> layer is like <code>modules</code> on upper level
      </p>

      <LazyImg path="/imgs/frontend-architecture/02 07 pages.png" width="400px" />

      <p>
        <code>page</code> may have own components, apis, helpers, etc...
      </p>
      ...
      <ul>
        <li>
          <code>page</code> should be as thin as possible
        </li>
        <li>ideally it should be just a list of modules and components</li>
        <li>
          as much business logic, helpers, states etc... should be extracted down into modules if
          possible
        </li>
      ...
      <ul>
        <li>
          in real project it is not enough just to have <code>pages</code>, <code>modules</code>,{' '}
          <code>components</code> & <code>ui</code>
        </li>
        <li>
          more global folders will be introduced, like <code>helpers</code>, <code>state</code>,{' '}
          <code>config</code>, <code>consts</code> etc...
        </li>
      </ul>
      ...
      <ul>
        ...
        <li>
          - what do we do if one module should be used in other module, group logic on a page level,
          but what if we need it on different pages, then we have to duplicate it
        </li>
        <li>
          - where to keep business related entities like data, types & components for a product, an
          article, a user
        </li>
      ...
      <p>
        Modular architecture is a way better than classical, but does not fit well for a project
        with complex business logic and big variety of features
      </p>
      ...
      <Lnk path="https://feature-sliced.design/docs/get-started/overview">
        https://feature-sliced.design/docs/get-started/overview
      </Lnk>

      <ul>
        <li>
          in FSD, a project consists of <code>layers</code>, <code>slices</code> and{' '}
          <code>segments</code>
        </li>
      </ul>
      ...
      <ul>
        <li>
          <code>/shared</code> dummy ui and utils without any business logic
        </li>
        <li>
          <code>/entities</code> THINGS which have shape, crud apis, components, internal business
          logic
        </li>
        <li>
          <code>/features</code> ACTIONS we apply to THINGS to gain some business value
        </li>
        <li>
          <code>/widgets</code> combination of shared + entities + features to an independent
          ready-made blocks, minimum business logic
        </li>
        <li>
          <code>/pages</code> component which has its own route (layout + widgets), no business
          logic here
        </li>
        <li>
          <code>/app</code> app-wide settings, providers, router, store
        </li>
      </ul>
      ...
      <ul>
        <li>
          This allows to do complex front end apps and it will not become a mess where all is
          cross-imported.
        </li>
        <li>
          It may be an overkill for small front end apps or for apps where all business logic is in
          the backend.
        </li>
      </ul>
      ...
      <ul>
        ...
        <li>
          e.g. create-blog-post, login-by-auth, edit-account, publish-video, show-ad, grant-access,
          show-contact-information
        </li>
        <li>
          may contain interactive UI elements, internal state and API calls that enable
      ...
      <ul>
        <li>
          compositional layer to combine lower-level units from entities + features into meaningful
          assembled blocks with content and interactive buttons wired to the api calls
        </li>
        <li>e.g. PostCard, IssuesList, UserProfile</li>
        <li>
          in this layer we fill slots left in the UI of Entities with other Entities and interactive
          elements from Features
        </li>
        ...
        <li>
          ❗️ it might be hard to decide what goes into Entities and Features. Do not worry. Just
          put all logic into Widgets layer. You will feel later if it should be split into Entities
          and Features
        </li>
      </ul>
      ...
      <ul>
        ...
        <li>
          <code>shared</code> and <code>app</code> layers never have slices as they do not have
          business logic inside
        </li>
        <li>❗️ slices cannot use other slices on the same layer</li>
        <li>
          Closely related slices can be grouped in a folder, but they still should follow rule above
        </li>
        <li>
          ❗️ slices (and segments without slices) must contain the <code>index</code> file entry
          points (public API) with module re-exports
        </li>
      ...
      <p>
        a slice consists of segments to separate code by its technical nature, common segments, ❗️
        but not necessarily are
      </p>

      <ul>
        <li>
          <code>ui</code> ui-logic, components
        </li>
        <li>
          <code>model</code> business logic, store, actions, selectors
        </li>
        <li>
          <code>lib</code> utils, helpers, hooks
        </li>
        <li>
          <code>api</code> communication with external APIs, backend API methods
        </li>
      </ul>
    </>
  )
