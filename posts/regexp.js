...
const postObj = {
  ...
  body: (
    <>
      ...
      <ul>
        <li>Via object <Code>const regexp = new RegExp("pattern", "flags")</Code></li>
        <li>Via slashes <Code>const regexp = /pattern/gmi</Code></li>
        ...
        <li><Code>new RegExp</Code> is more often used when we need to create a regexp “on the fly”</li>
      ...
      <ol>
        ...
        <li><Code>/.../g</Code> all matches</li>
        <li><Code>/.../m</Code> multiline mode</li>
        <li><Code>/.../s</Code> match newline character \n</li>
        <li><Code>/.../u</Code> full Unicode support (correct processing of surrogate pairs)</li>
        <li><Code>/.../y</Code> searching at the exact position in the text</li>
      </ol>
      ...
      <Hs><Lnk path="https://unicode.org/reports/tr18/#General_Category_Property">General category</Lnk></Hs>
      ...
      <Hs><Lnk path="https://tc39.es/ecma262/multipage/text-processing.html#table-unicode-script-values">Script</Lnk></Hs>

      <Code block jsx>{`
            "Привет man".match(/\\p{Script=Cyrillic}/gu) // ['П', 'р', 'и', 'в', 'е', 'т']
      `}</Code>

      <Hs><Lnk path="https://tc39.es/ecma262/multipage/text-processing.html#table-binary-unicode-properties">Binary Unicode property</Lnk></Hs>

      <p>ASCII, ASCII_Hex_Digit, Alphabetic, Any, Dash, Emoji, Hex_Digit, Lowercase, Math, Noncharacter_Code_Point, Pattern_Syntax, Pattern_White_Space, Quotation_Mark, Radical, Regional_Indicator, Sentence_Terminal, Soft_Dotted, Terminal_Punctuation, Unified_Ideograph, Uppercase, White_Space</p>
      ...
      <p><code>{'['}</code> <code>\</code> <code>{'^'}</code> <code>{'$'}</code> <code>{'.'}</code> <code>{'|'}</code> <code>{'?'}</code> <code>{'*'}</code> <code>{'+'}</code> <code>{'('}</code> <code>{')'}</code> <code>{'.'}</code> to be escaped with backslash <code>\</code></p>
      ...
      <p>In square brackets we can use the vast majority of special characters w/o escaping, until they mean something for brackets</p>

      <H>Quantifiers <code>+</code>, <code>*</code>, <code>?</code>, <code>{'{n}'}</code></H>
      ...
      <H>Capturing groups <code>(...)</code></H>
      ...
      <H>Capturing groups & <Code>str.replace()</Code></H>
      ...
      <p>Exclude group with <code>?:</code></p>
      ...
      <p>Named groups & <Code>str.replace()</Code></p>
      ...
      <p>Named groups can be used <Code> {'\\k<name>'}</Code></p>

      <Code block jsx>{`
            \`He said: "She's the one!".\`.match(/(?<quote>['"])(.*?)\\k<quote>/g) // ["She's the one!"]
      `}</Code>

      <H>Lookahead <code> (?= smth)</code></H>
      ...
      <H>Negative lookahead <Code> (?!= smth)</Code></H>
      ...
      <H>Lookbehind <Code> {'(?<=Y)X'}</Code></H>

      <p><code>{'(?<=Y)X'}</code> matches X only if there’s Y before it</p>

      <Code block jsx>{`
            "1 turkey costs $30".match(/(?<=\\$)\\d+/) // '30'
      `}</Code>

      <H>Negative lookbehind <Code> {'(?<!Y)X'}</Code></H>

      <p><code>{'(?<!Y)X'}</code> matches X if there’s no Y before it</p>
      ...
      <H>Search at position with 'y' flag <Code> {'/.../y'}</Code></H>

      <ul>
        <li>Flag 'y' allows to perform the search at the given position in the source string</li>
        <li><Code>regexp.exec(str)</Code> works like <Code>str.match(regexp)</Code></li>
        ...
        <li>Successive calls to <Code>regexp.exec(str)</Code> return matches one after another</li>
      </ul>
      ...
      <p>'y' flag makes <Code> regexp.exec()</Code> to search exactly at position <Code> lastIndex</Code></p>
      ...
      <ol>
        <li><Code>str.match(regexp)</Code> - finds all matches of 'regexp' in the string 'str', with 'g' flag returns an array</li>
        <li><Code>str.matchAll(regexp)</Code> - returns not an array, but an iterable object</li>
        <li><Code>str.split(regexp, limit)</Code> - Splits the string using the regexp (or a substring) as a delimiter</li>
        <li><Code>str.search(regexp)</Code> - returns the position of the first match or -1 if none found:</li>
        <li><Code>str.replace(regexp, replacement)</Code> - replaces matches found using regexp in string str with replacement</li>
        <li><Code>str.replaceAll(regexp, replacement)</Code> - same as str.replace With 'g' flag </li>
        <li><Code>regexp.exec(str) </Code> - works exactly like <Code>str.match(regexp)</Code></li>
        <li><Code>regexp.test(str)</Code> - looks for at least one match, if found, returns true, otherwise false</li>
      </ol>
      ...
      <ul>
        ...
        <li>To ensure a result to be an array put <Code>let result = str.match(regexp) || []</Code></li>
      </ul>
      ...
      <ul>
        ...
        <li>If we need positions of all matches, use <Code>str.matchAll(regexp)</Code></li>
      </ul>
      ...
      <ul>
        ...
        <li><Code>replacementFunc(str, offset, input)</Code></li>
      ...
      <ul>
        <li>Same as <Code>str.replace()</Code>, with two major differences</li>
        ...
        <li>With 'g' flag, it works the same as <Code>str.replace()</Code></li>
      ...
      <ul>
        ...
        <li>If no 'g', then returns the first match, same as <Code>str.match(regexp)</Code></li>
        ...
        <li>And so on...</li>
        <li>If there are no matches, <Code>regexp.exec()</Code> returns null and resets <code>regexp.lastIndex</code> to <code>0</code></li>
      ...
      <ul>
        ...
        <li>If the regexp has flag 'g', then looks from <code>regexp.lastIndex</code> property and updates this property, just like <Code>regexp.exec()</Code></li>
