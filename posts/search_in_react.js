...
export const getJsxWithBoldSubstr = (text, subString) => {
  ...
  return text
    .split(regExp)
    .map((str, index) => (regExp.test(str) ? <b key={`char-${index}`}>{str}</b> : str))
}
...
function Component() {
  ...
  useEffect(() => {
    ...
    const foundCountries = countriesAll.filter((country) =>
      country.toLowerCase().includes(inputValue.toLocaleLowerCase().trim())
    )
    syncWait(500)
    ...
    <>
      ...
      <ul>
        {countries.map((country) => (
          <li key={country}>
            {inputValue ? <span>{getJsxWithBoldSubstr(country, inputValue)}</span> : country}
          </li>
        ))}
      </ul>
    </>
...
const postObj = {
  ...
  body: (
    <>
      ...
      <ul>
        <li>
          we put some 500ms delay before ui update via <code>syncWait</code> function
        </li>
        <li>
          in case <code>useEffect</code> depends directly on <code>inputValue</code> we will
          experience sluggishness
        </li>
        ...
        <li>
          as soon as our <code>useEffect</code> depends on <code>deferredInputValue</code> we have
          less ui freezes
        </li>
        <li>
          <code>deferredInputValue</code> signals the react that this state has low priority and
          should be rendered only after main important renders, like input values update
        </li>
      </ul>
