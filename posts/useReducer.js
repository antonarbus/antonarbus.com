...
const postObj = {
  ...
  body: (
    ...
    <p>
      The principle for <Code js >useReducer()</Code> hook is similar to <i>Redux</i>, but
      state is not an object, but a pure value & action value is not an object with <i>type</i>{' '}
      property, but pure string.
      </p>

    <p>
      With <Code js >useReducer()</Code> instead of having multiple <Code js>setState()</Code> in different
      functions, we can have them all in one place.
      </p>
  ...
  `}</Code>
    ...
    <H>One <Code>useReducer()</Code> for multiple components</H>
    ...
    <H><Code>useReducer()</Code> with state & action as objects</H>

    <p>Same approach, but <i>state</i> and <i>actions</i> are enhanced.</p>
