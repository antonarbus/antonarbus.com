...
const postObj = {
  ...
  body: (
    <>
      ...
      <ul>
        <li><Code js>const d = new Date()</Code> returns the date object</li>
        <li>getters like <Code js>d.getHours()</Code> return local time with GMT shift based on client's device</li>
        <li>getters like <Code js>d.getUTCHours()</Code> return UTC time</li>
        <li><Lnk path='/api/time_on_server'>API call</Lnk> returns the date object parsed on server side with <code>0</code> timezone shift</li>
        <li>Same object on the client side returns local time with local timezone</li>
        <li>UTC time is the same on both sides</li>
        <li>Do not rely on server local time if you present time to a client, take the date object and parse it on client's device</li>
      </ul>
